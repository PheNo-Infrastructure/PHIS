# OpenSILEX GitHub Installation Script v1.4.9-rdg (Modular Version)
...
param(
    ...
    [Parameter(Mandatory=$false)]
    [string]$VMName = "phis-debian12-TEST-NEW",
    
    [Parameter(Mandatory=$false)]
    [string]$ResourceGroupName = "RG-OPENSILEX-debian12-TEST-NEW",
...
Import-Module -Name "$ModulePath\OpenSILEX-Installer.psm1" -Force
...
try {
    ...
    switch ($Command.ToLower()) {
        "menu" { 
            $choice = ""
            while ($choice -ne "0") {
                $choice = Show-Menu -VMName $VMName -ResourceGroupName $ResourceGroupName -Location $Location -AdminUsername $AdminUsername -SSHKeyPath $SSHKeyPath -SkipDependencies:$SkipDependencies -PSScriptRoot $PSScriptRoot
                if ($choice -ne "0") {
                    Write-Host ""
        ...
        default {
            Invoke-MenuCommand -Command $Command -VMName $VMName -ResourceGroupName $ResourceGroupName -Location $Location -AdminUsername $AdminUsername -SSHKeyPath $SSHKeyPath -VMIPAddress $VMIPAddress -SkipDependencies:$SkipDependencies -PSScriptRoot $PSScriptRoot
        }
    }
} catch {
